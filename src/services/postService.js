...
export const votePost = async (postId, authToken) => {
  try {
    await axios.post(process.env.REACT_APP_BACKEND_URL + `/api/post/${postId}/vote`, null, {
...
export const bookmarkPost = async (postId, authToken) => {
  try {
    const response = await axios.post(process.env.REACT_APP_BACKEND_URL + `/api/user/${postId}/bookmark`, null, {
      headers: { authorization: authToken },
    });
    return response.data;
  } catch (err) {
...
export const getPostFilters = async () => {
  try {
    const response = await axios.get(process.env.REACT_APP_BACKEND_URL + '/api/post/filters');
    return response.data;
  } catch (err) {
    throw new Error(err.response.data.error);
  }
...
export const getSuggestedPosts = async (authToken, offset = 0) => {
  try {
    const response = await axios.get(process.env.REACT_APP_BACKEND_URL + '/api/post/suggested/' + offset, {
      headers: {
        authorization: authToken,
      },
    });
...
export const getHashtagPosts = async (authToken, hashtag, offset = 0) => {
  try {
    const response = await axios.get(process.env.REACT_APP_BACKEND_URL + `/api/post/hashtag/${hashtag}/${offset}`, {
      headers: {
        authorization: authToken,
      },
    });
