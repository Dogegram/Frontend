...
export const followUser = async (userId, authToken) => {
  try {
    const response = await axios.post(process.env.REACT_APP_BACKEND_URL + `/api/user/${userId}/follow`, null, {
...
export const retrieveUserFollowing = async (userId, offset, authToken) => {
  try {
    const response = await axios.get(
      process.env.REACT_APP_BACKEND_URL + `/api/user/${userId}/${offset}/following`,
      {
        headers: { authorization: authToken },
      }
    );
...
export const retrieveUserFollowers = async (userId, offset, authToken) => {
  try {
    const response = await axios.get(
      process.env.REACT_APP_BACKEND_URL + `/api/user/${userId}/${offset}/followers`,
      {
        headers: { authorization: authToken },
      }
    );
