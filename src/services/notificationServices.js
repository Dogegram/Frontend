...
export const retrieveNotifications = async (authToken) => {
  try {
    const response = await axios.get(process.env.REACT_APP_BACKEND_URL + '/api/notification', {
      headers: {
...
export const readNotifications = async (authToken) => {
  try {
    await axios.put(process.env.REACT_APP_BACKEND_URL + '/api/notification', null, {
      headers: {
        authorization: authToken,
      },
    });
