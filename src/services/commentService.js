...
export const deleteComment = async (commentId, authToken) => {
  try {
    await axios.delete(process.env.REACT_APP_BACKEND_URL + `/api/comment/${commentId}`, {
      headers: {
        authorization: authToken,
      },
    });
...
export const voteComment = async (commentId, authToken) => {
  try {
    await axios.post(process.env.REACT_APP_BACKEND_URL + `/api/comment/${commentId}/vote`, null, {
...
export const createCommentReply = async (
  ...
  try {
    const response = await axios.post(
      process.env.REACT_APP_BACKEND_URL + `/api/comment/${parentCommentId}/reply`,
      { message },
      {
        headers: {
          authorization: authToken,
...
export const deleteCommentReply = async (commentReplyId, authToken) => {
  try {
    await axios.delete(process.env.REACT_APP_BACKEND_URL + `/api/comment/${commentReplyId}/reply`, {
      headers: {
        authorization: authToken,
      },
    });
...
export const voteCommentReply = async (commentReplyId, authToken) => {
  try {
    await axios.post(process.env.REACT_APP_BACKEND_URL + `/api/comment/${commentReplyId}/replyVote`, null, {
      headers: { authorization: authToken },
    });
  } catch (err) {
    throw new Error(err.response.data.message || err.response.data.error || err.response.data || err);
...
export const getCommentReplies = async (parentCommentId, offset = 0) => {
  try {
    const response = await axios.get(
      process.env.REACT_APP_BACKEND_URL + `/api/comment/${parentCommentId}/${offset}/replies`
    );
    return response.data;
  } catch (err) {
...
export const getComments = async (postId, offset, exclude = 0) => {
  try {
    const response = await axios.get(
      process.env.REACT_APP_BACKEND_URL + `/api/comment/${postId}/${offset}/${exclude}`
    );
    return response.data;
  } catch (err) {
    throw new Error(err.response.data.message || err.response.data.error || err.response.data);
